...
以下操作适用于已经安装飞牛影视 v2.6.2、希望使用本项目个性化配置的用户。

操作前请先完全退出飞牛影视。不要把配置复制到飞牛影视的安装目录，
而应复制到当前用户的 mpv 配置目录：

- Windows：`%APPDATA%\mpv`
- macOS：`~/.config/mpv`

下面的命令会把 `custom_config/mpv` 和 `custom_config/uosc_danmaku` 里的内容复制过去。
已有的同名文件会被覆盖，目录中的其他文件不会被删除。
